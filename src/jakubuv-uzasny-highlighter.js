...
class objectElement {
  ...
  accept() {
    ...
    const evn = new Event("input", { bubbles: true, cancelable: true, view: window });
...
function defineRoot(textElement) {
  ...
  linkStyle.setAttribute("href", korekthor_ajax.plugin_url + "/assets/highlight.css");
...
function setDimensions(from, to, window) {
...
function setup(element, underlineObjects, window) {
  ...
  setDimensions(element, underlineWindow, window);
...
function setupError(error, mainElement, underlineContainer, root, window) {
  const [node, offset] = getNode(error.index, mainElement, window);
  ...
  return createUnderline(offset, node, underlineContainer, root, count, error.id, window);
}

function getNode(index, original, window) {
  const data = getBlocks(original, window);
...
function getBlocks(element, window) {
...
function createUnderline(offset, node, underlineContainer, root, count, id, window) {
...
export function main(element, content, mistakes, sendObj) {
  ...
  if (founded_processed_element === undefined) {
    founded_processed_element = setup(element, underlineObjects, window);
  ...
  const ro = new ResizeObserver((entry) => {
    ...
    if (rect.width !== 0) {
      ...
      const errors = getErrors(content, mistakes);
      ...
      errors.forEach((error) => {
        const [underlines, range] = setupError(error, element, underlineContainer, root, window);
